...
func saveRoomMessagesToFile(roomId string, roomName string) error {
    mutex.Lock()
    defer mutex.Unlock()
    ...
    fileName := fmt.Sprintf("%s_%s_messages.json", roomId, roomName) // roomId 포함
    ...
    fmt.Printf("채팅방 '%s' (ID: %s)의 메시지가 %s 파일에 저장되었습니다.\n", roomName, roomId, fileName)
    return nil
}

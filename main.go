...
// 	fileName := fmt.Sprintf("%s_%s_messages.json", roomIdStr, roomName)
...
func main() {
	...
	r.POST("/messages", func(c *gin.Context) {
		collection := db.GetCollection("chatApp", "messages")
		...
		result, err := collection.InsertOne(nil, message)
		if err != nil {
			c.JSON(500, gin.H{"error": "Failed to insert message"})
			return
		}

		c.JSON(200, gin.H{"insertedID": result.InsertedID})
	})
